# -*- coding: utf-8 -*-
"""
Created on Tue Aug 18 00:26:32 2020

@author: shadow
"""
"""
1. Load data
2. Keep the cat vars in a variable categorical = df.dtypes[df.dtypes == 'object'].index
3. Check which are low/high cardinality
    high_cardinality = [col for col in x[categorical]] if x[col].nunique() > 10]
    low_cardinality = [col for col in x[categorical]] if x[col].nunique() <= 10]

4. For the high_cardinality columns we do a simple LabelEncoder
    label_encoder = LabelEncoder()
    for col in high_cardinality:
        label_X_train[col] = label_encoder.fit_transform(X_train[col])
5. For the low_cardinality we do a OneHotEncoder (1 columns per unique value) 
    OH_cols_train = pd.DataFrame(encoder.fit_transform(X_train[low_cardinality_cols]))
5.1 Put the indices back to the encoded columns as they were removed
    OH_cols_train.index = X_train.index
5.2 Remove the categorical columns from the original dataset
    num_X_train = X_train.drop(object_cols, axis=1)
5.3 Concat them two dfs
...
without preprocessing them first. In this tutorial, we'll compare three approaches that 
you can use to prepare your categorical data.
...
categories. Not all categorical variables have a clear ordering in the values, but we refer to those 
that do as ordinal variables. For tree-based models (like decision trees and random forests), 
you can expect label encoding to work well with ordinal variables.

3) One-Hot Encoding
One-hot encoding creates new columns indicating the presence (or absence) of each 
possible value in the original data. To understand this, we'll work through an example.
"""
...
indicates a column has text (there are other things it could theoretically be, 
but that's unimportant for our purposes). For this dataset, the columns with 
text indicate categorical variables.
"""
...
#to a different integer. This is a common approach that is simpler than providing custom labels; 
...
since it had the highest MAE score. As for the other two approaches, 
since the returned MAE scores are so close in value, there doesn't appear to be 
any meaningful benefit to one over the other.

In general, one-hot encoding (Approach 3) will typically perform best, and dropping 
the categorical columns (Approach 1) typically performs worst, but it varies on
